-- Add client_id to campaigns. Ties happen at campaign level: advertiser, agency, client.
ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS client_id uuid REFERENCES clients(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS idx_campaigns_client_id ON campaigns(client_id);

-- get_all_client_counts: count via campaign.client_id (not agency).
...
BEGIN
  FOR r IN SELECT id FROM clients LOOP
    SELECT count(DISTINCT c.agency_id)::int INTO v_agency_count
    FROM campaigns c WHERE c.client_id = r.id AND c.agency_id IS NOT NULL;

    SELECT count(DISTINCT c.advertiser_id)::int INTO v_advertiser_count
    FROM campaigns c WHERE c.client_id = r.id;
